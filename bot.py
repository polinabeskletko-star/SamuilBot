...
# Память диалогов Самуила по чатам
DIALOG_HISTORY: dict[int, list[dict[str, str]]] = {}
...
def get_history(chat_id: int) -> list[dict[str, str]]:
    return DIALOG_HISTORY.setdefault(chat_id, [])


def append_history(chat_id: int, role: str, content: str):
    history = DIALOG_HISTORY.setdefault(chat_id, [])
...
async def call_openai(
    ...
    """
    Обёртка над OpenAI Chat Completions. Возвращает (text, error_message).
    """
    if client is None:
...
async def generate_image_bytes(prompt: str) -> tuple[bytes | None, str | None]:
    """
    Генерация картинки через OpenAI Images.
    Возвращает (байты PNG, сообщение об ошибке или None).
    """
    if client is None:
        return None, "OpenAI client is not configured (no API key)."
...
async def generate_message_for_kind(
    ...
    kind:
      ...
      - "samuil_qa"         — длинный ответ Самуила на вопрос
      - "weekend_hourly"    — часовой вопрос по выходным (если ещё используется)
      - "weekday_morning"   — утреннее сообщение по будням (если ещё используется)
    """
    ...
    if kind == "samuil_qa":
        # Диалог с Самуилом — используем историю
        if chat_id is None:
            return None, "chat_id is required for samuil_qa"

        history = get_history(chat_id)

        system_prompt = (
            "Ты умный, ироничный, но доброжелательный помощник по имени Самуил в Telegram-чате. "
            "Ты отвечаешь по-русски, можешь быть немного саркастичным, но не злым. "
            "Отвечай по существу, можешь структурировать ответ пунктами, если вопрос сложный. "
            "Ты видишь только текстовые сообщения из чата, не придумывай фактов о реальных людях, "
            "если они не следуют прямо из контекста."
        )
        user_prompt = (
            f"Сегодня {weekday_name}, время {time_str}. "
            f"Тебе в чате задали такой вопрос или обратились к тебе: «{user_text}».\n"
            "Дай развёрнутый, полезный ответ. Если уместно — немного иронии."
        )
        # Дадим Самуилу побольше длины
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "private":
        await update.message.reply_text(
            ...
            "Если в чате написать «Самуил, ...», я попробую ответить как умный ИИ.\n"
            "А если попросить у Самуила картинку, я попробую её сгенерировать."
        )
    else:
        await update.message.reply_text(
            "Я здесь, чтобы поддерживать Максима и иногда отвечать как Самуил.\n"
            "• Будни: сообщение в 7:00.\n"
            "• Выходные: раз в час в случайную минуту.\n"
            "• Если в сообщении есть слово «Самуил», я воспринимаю это как вопрос.\n"
            "• Если в сообщении есть «Самуил» и «картинку/фото/изображение», я попробую прислать картинку.\n"
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- 1. Запрос к Самуилу на КАРТИНКУ ---
    if "самуил" in lower_text and any(k in lower_text for k in IMAGE_KEYWORDS):
        # Простейший промпт: делаем картинку по описанию всего сообщения
        img_prompt = (
            "Создай дружелюбную, забавную картинку-мем без жестокости и пошлости, "
            f"основанную на этом описании из чата: «{text}». "
            "Стиль — лёгкий юмор, ничего шокирующего."
        )
        img_bytes, err = await generate_image_bytes(img_prompt)
        if img_bytes is None:
            # Фоллбек — текст вместо картинки
            fallback = "Сегодня Самуил без кисточки: с картинкой не вышло, но идея была классная."
            print(f"OpenAI image error: {err}")
            await message.chat.send_message(fallback)
            return

        bio = BytesIO(img_bytes)
        bio.name = "samuil_image.png"
        try:
            await message.chat.send_photo(
                photo=bio,
                caption="Самуил нарисовал это специально для тебя.",
            )
        except Exception as e:
            print("Error sending image to Telegram:", e)
            await message.chat.send_message(
                "Я картинку сделал, но Телеграм что-то не оценил. Представь, что она была очень смешная."
            )
        # Можно при желании запоминать это как ответ Самуила
        append_history(chat_id, "user", text)
        append_history(chat_id, "assistant", "[Самуил отправил картинку]")
        return

    # --- 2. Обычный текстовый диалог с Самуилом ---
    if "самуил" in lower_text:
        append_history(chat_id, "user", text)

        ai_text, err = await generate_message_for_kind(
            "samuil_qa", now=now, user_text=text, chat_id=chat_id
        )
        if ai_text is None:
            fallback = "Самуил сейчас завис как настоящий ИИ. Попробуй спросить ещё раз попроще 🙂"
            print(f"OpenAI error for samuil_qa: {err}")
            await message.chat.send_message(fallback)
            return

        append_history(chat_id, "assistant", ai_text)
        await message.chat.send_message(ai_text)
        return

    # --- 3. Сообщения Максима — саркастичный ответ ---
...
# ---------- SCHEDULED JOBS (примерно как раньше) ----------
